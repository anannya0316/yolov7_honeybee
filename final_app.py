...
def download_images_to_folder(image_keys):
    """Download images from S3 and save them to a local folder."""
    
    # Define the path for the directory to store images
    images_folder = os.path.join(get_downloads_folder(), "wrong_classification_images")
    ...
    for key in image_keys:
        try:
            ...
            image_path = os.path.join(images_folder, os.path.basename(key))

            # Save the image
            img.save(image_path)
            st.success(f"Image {os.path.basename(key)} saved successfully!")
            ...
            if not filtered_keys:
                ...
                with col2:
                    ...
                    if not bad_image_keys:
                        ...
                        download_images_to_folder(bad_image_keys)
