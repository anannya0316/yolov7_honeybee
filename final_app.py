...
def extract_all_dates(keys):
    ...
    for key in keys:
        if not key.startswith('qu13edjkbs'):  # Only consider keys without 'qu13edjkbs' prefix
            try:
                # Attempt to extract a date substring from the key
                date_str = key.split('_')[1][:8]  # Adjust index based on your naming convention
                date_obj = datetime.strptime(date_str, "%Y%m%d")
                dates.add(date_obj.strftime('%Y-%m-%d'))
            except (IndexError, ValueError):
                continue  # Skip if unable to extract a date
    return sorted(dates)
    ...
    if not st.session_state.get('authenticated', False):
        ...
        if uploaded_file is not None:
            ...
            try:
                ...
                # Print 'no objects are detected' if no labels are found
                if not labels:
                    st.write("No objects are detected.")
                else:
                    st.write(labels)
...
if selected_tab == "🧠 Training":
    if not st.session_state.get('authenticated', False):
        ...
        st.header("🧠 YOLOv7 Training Setup")

        # Upload zip file with train, test, valid folders
        uploaded_zip = st.file_uploader("Upload a ZIP file with training data", type=["zip"])

        if uploaded_zip is not None:
            # Define the path for extracting the zip file
            extract_path = os.path.join(get_downloads_folder(), "yolov7_training_data")
            ...
            st.success(f"Training data extracted to: {extract_path}")

            # Display extracted files and directories
            for root, dirs, files in os.walk(extract_path):
                for file in files:
                    st.write(os.path.join(root, file))

            # Additional training logic can be added here
            # You can display, analyze, or process the files as needed
